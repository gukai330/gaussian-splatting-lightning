...
from .gsplat_v1_renderer_pano import GSplatV1RendererModule, GSplatV1
from .gsplat_contrastive_feature_renderer_pano import GSplatContrastiveFeatureRenderer
...
from internal.viewer.ui.multiple_bounding_boxes_panel import MultipleBoundingBoxesPanel # Import the MultipleBoundingBoxesPanel
from viser import ViserServer

from internal.utils.common import parse_cfg_args
...
class SegAnyGSRenderer(GSplatV1RendererModule):
    def __init__(
            self,
        ...
        # Pass a dummy config to the parent, as we don't use its config directly
        super().__init__(config=type('obj', (object,), {'anti_aliased': anti_aliased, 'separate_sh': False, 'tile_based_culling': False, 'max_viewspace_grad_scale': 65535.})())
        ...
        self.color_producers = {
            ...
            "acc_depth": self._depth_as_color,
    ...
    def forward(
        ...
        projections = GSplatV1.project(
            ...
            near_plane=self.runtime_options.near_plane,
        ...
        def rasterize(input_features: torch.Tensor, background, return_alpha: bool = False):
            ...
            if return_alpha:
                return rendered_colors.squeeze(0), rendered_alphas.squeeze(0).squeeze(-1)
            return rendered_colors.squeeze(0)

        outputs = {}

        for i in render_types:
            colors, rasterize_bg_color, new_opacities = self.color_producers[i](projections, pc, viewpoint_camera, bg_color, opacities.squeeze(0))
            outputs[self.available_output_types[i]] = rasterize(colors, rasterize_bg_color, return_alpha=(i == "alpha"))
            if i != "alpha":
                 outputs[self.available_output_types[i]] = outputs[self.available_output_types[i]].permute(2, 0, 1)
        ...
        # Add additional outputs from GSplatV1RendererModule if needed
        # outputs["viewspace_points"] = means2d
        # outputs["viewspace_points_grad_scale"] = 0.5 * torch.tensor([preprocessed_camera[-1]]).to(means2d).clamp_(max=self.config.max_viewspace_grad_scale)
        # outputs["visibility_filter"] = visibility_filter
        # outputs["acc_vis"] = means2d.has_hit_any_pixels if hasattr(means2d, 'has_hit_any_pixels') else None # Check if attribute exists
        # outputs["radii"] = radii_squeezed
        # outputs["scales"] = scales
        # outputs["opacities"] = opacities[0]
        # outputs["projections"] = projections
        # outputs["isects"] = isects
    ...
    def _depth_as_color(self, project_results, pc: GaussianModel, viewpoint_camera, bg_color, opacities):
        # Use depths from project_results
        depths = project_results[2].squeeze(0).unsqueeze(-1)
        return depths, torch.zeros((1,), dtype=torch.float, device=bg_color.device), opacities
    ...
    def get_available_outputs(self) -> Dict:
        ...
        for i in self.available_output_types:
            # Avoid overriding existing keys from super() if they exist and are not depth maps
            if i not in available_outputs or self.is_type_depth_map(i):
...
class ViewerOptions:
    def __init__(
        ...
        cfg_args = self._parse_cfg_args(model_dir)
        self.source_path = cfg_args.source_path
        ...
        # Add Bounding Box Panel
        with server.gui.add_folder("Bounding Box"):
            self.bounding_box_panel = MultipleBoundingBoxesPanel(self.server, self.viewer, self)

    @classmethod
    def _parse_cfg_args(cls, model_path):

        segany_file = os.path.join(model_path, "seganygs")
        with open(segany_file, "r") as f:
            init_model_path = f.read().strip()

        return parse_cfg_args(os.path.join(init_model_path, "cfg_args"))
    ...
    def _setup_segment(self):
        ...
        def _(event):
            ...
            camera_model = self.renderer.runtime_options.camera_model

            # model string to type id
            camera_type = 0
            if camera_model == 'fisheye':
                camera_type = 1
            elif camera_model == 'panorama':
                camera_type = 3
            
            camera.camera_type = torch.tensor([camera_type], dtype=torch.int)
