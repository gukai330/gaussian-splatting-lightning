...
class RuntimeOptions:
    ...
    camera_model: Literal["pinhole", "ortho", "fisheye", "panorama", None ] =  None
...
class GSplatV1ViewerOptions:
    def __init__(self, viewer, server: ViserServer, options: RuntimeOptions):
        ...
        self.near_clip_number = server.gui.add_number(
            label="Near Clip",
            initial_value=options.near_plane,
            step=0.01,
            min=0.,
            max=1.,
        )
        
        @self.near_clip_number.on_update    
        def _(_):
            options.near_plane = self.near_clip_number.value
            viewer.rerender_for_all_client()
        ...
        self.camera_model_dropdown = server.gui.add_dropdown(
            label="Camera Model",
            options=["pinhole", "ortho", "fisheye", "panorama"],
