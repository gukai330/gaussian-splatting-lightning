...
class GaussianSplatting(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # image_name, gt_image, masked_pixels = image_info
        camera.camera_type = torch.tensor([3], dtype=torch.int, device=camera.R.device)
        ...
        # log learning rate and gaussian count every 2 iterations (without plus one step)
        if self.trainer.global_step % 2 == 0:        
            reg_metrics = self.regularize_step(batch, batch_idx, global_step)
            metrics["loss"] += reg_metrics["loss"]
    ...
    def regularize_step(self, batch, batch_idx, step):
        ...
        batch_persp = get_perspective_batch(batch)
        ...
        if (step-1)%981 == 2:
            ...
            gt_img_persp = image_info_persp[1].detach().permute(1,2,0).cpu().numpy() *255
            gt_img_persp = gt_img_persp.astype('uint8')

            render_persp = outputs["render"]
            render_persp = render_persp.detach().permute(1, 2, 0).cpu().numpy() * 255
            render_persp = render_persp.astype('uint8')

            print(gt_img_persp.shape, render_persp.shape)
            # save image
            import imageio


            imageio.imwrite('gt_persp.png', gt_img_persp)
            imageio.imwrite('render_persp.png', render_persp)

            # raise ValueError("save image")



        metrics, _ = self.metric.get_train_metrics(self, self.gaussian_model, self.trainer.global_step, batch_persp, outputs)
...
from internal.utils.equirect_utils import equirect2persp
import math

def get_perspective_batch(batch):
    ...
    output_width = camera.width // 2
    output_height = camera.height 
   
    fov = 120
    # calculate the fx fy
    fx = torch.tensor(fov2focal(math.radians(fov), output_width), device=camera.device)
 

    persp_camera = Cameras(
        R = camera.R.unsqueeze(0),
        T = camera.T.unsqueeze(0),
        fx = fx.unsqueeze(0),
        fy = fx.unsqueeze(0),
        cx = torch.tensor([output_width / 2], device=camera.device).unsqueeze(0),
        cy = torch.tensor([output_height / 2], device=camera.device).unsqueeze(0),
        width = torch.tensor([output_width], device=camera.device).unsqueeze(0),
        height = torch.tensor([output_height], device=camera.device).unsqueeze(0),
        appearance_id = camera.appearance_id.unsqueeze(0),
        normalized_appearance_id = camera.normalized_appearance_id.unsqueeze(0),
        distortion_params= None,
        camera_type= torch.tensor([0], dtype = torch.int).unsqueeze(0),
    )
   ...
   # 
    # persp_camera.fx = fx
    # persp_camera.fy = fx
    # persp_camera.fov_x = torch.tensor(fov, device=persp_camera.device)
    # persp_camera.fov_y = torch.tensor(fov, device=persp_camera.device)
    # persp_camera.cx = output_width / 2
    # persp_camera.cy = output_height / 2

 
    # stack the gt image and the masked pixels
    stacked_img = torch.cat([gt_image, masked_pixels], dim = 0)

    # convert the gt image to perspective
    stacked_persp_img = equirect2persp(stacked_img.unsqueeze(0),
                                        fov, theta=0, phi=0,
                                        hd = output_height.item(), 
                                        wd = output_width.item()).squeeze(0)

    # split the gt image and the masked pixels
    gt_img_persp = stacked_persp_img[ :3, :, :]
    masked_pixels_persp = stacked_persp_img[ 3:, :, :]
    # make new batch
    persp_batch = (persp_camera[0].to_device(camera.device), (image_name, gt_img_persp, masked_pixels_persp), None)    
