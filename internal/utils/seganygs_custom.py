...
class SegAnyGSUtils:
    ...
    def get_segment_mask_by_raw_feature_list(
        ...
        scale_conditioned_semantic_features: torch.Tensor,
        query_features: List,
        scale_gate: ScaleGateUtils = None,
        scale: float = 1.0,
        score: float = 0.8,
        gamma: float = None,
        ...
        positive_list = [f for f, label in query_features if label is True]
        negative_list = [f for f, label in query_features if label is False]

        if positive_list:
            # positive相似度
            positive_similarities = cls.get_similarities_by_raw_feature_list(
            ...
            if gamma is not None and gamma != 1.0:
                positive_similarities = torch.pow(positive_similarities + gamma_eps, gamma)

            pos_mask = (positive_similarities >= score).sum(dim=-1) > 0

        else:
            pos_mask = torch.zeros(scale_conditioned_semantic_features.shape[0], dtype=torch.bool).to(scale_conditioned_semantic_features.device)
            positive_similarities = previous_similarity

        pos_mask |= previous_mask if previous_mask is not None else pos_mask


        if not negative_list:
            if return_similarity_matrix:
                return pos_mask, positive_similarities
            return pos_mask

        # 只在positive筛选过的点中计算negative相似度
        if pos_mask.sum() == 0:
            pos_mask = torch.ones_like(pos_mask, dtype=torch.bool)

        filtered_features = scale_conditioned_semantic_features[pos_mask]

        negative_similarities = cls.get_similarities_by_raw_feature_list(
            filtered_features,
            negative_list,
            scale_gate,
            scale,
        ...
        if gamma is not None and gamma != 1.0:
            negative_similarities = torch.pow(negative_similarities + gamma_eps, gamma)


        # maybe use a different score later
        neg_mask_filtered = (negative_similarities >= score*1.05).sum(dim=-1) > 0

        # 更新positive mask，只保留未被negative排除的
        final_mask = pos_mask.clone()
        final_mask[pos_mask] = ~neg_mask_filtered

        if return_similarity_matrix:
            # 注意negative_similarities只对应positive筛选过的点
            return final_mask, positive_similarities
